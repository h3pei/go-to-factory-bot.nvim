...
local function jump_from_factory_call()
  ...
  if not FactoryNameExtractor.is_available() then
    notify("Treesitter is not available. Please install nvim-treesitter and Ruby parser (:TSInstall ruby)", vim.log.levels.ERROR)
    return
  end
...
local function jump_from_model_file()
  local model_name, namespace, error_message = ModelPathAnalyzer.extract_model_info()
  if not model_name then
    notify(error_message or "Failed to extract model", vim.log.levels.WARN)
    return
  end

  -- 名前空間を考慮して検索
  local factory_path = FactoryPathFinder.find_by_name(model_name, namespace)
  if not factory_path then
